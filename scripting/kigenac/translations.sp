...
//	SetTrieValue(g_hLanguages, "nl", any:CreateTrie());
	...
	SetTrieString(f_hTemp, KAC_HASPLUGIN, 		"%s (%s) has a plugin running, returned %s.");
	SetTrieString(f_hTemp, KAC_MUTED, 		"%s has been muted by Kigen's Anti-Cheat.");
	SetTrieString(f_hTemp, KAC_HASNOTEQUAL, 	"%s (%s) returned a bad value on %s (value %s, should be %s).");
	...
	SetTrieString(f_hTemp, KAC_HASNOTGREATER,	"%s (%s) has convar %s set to %s when it should be greater than or equal to %s.");
	...
	SetTrieString(f_hTemp, KAC_HASNOTLESS,		"%s (%s) has convar %s set to %s when it should be less than or equal to %s.");
	...
	SetTrieString(f_hTemp, KAC_HASNOTBOUND,		"%s (%s) has convar %s set to %s when it should be beteween %s and %f.");
